...
#[test]
fn record_snapshot_returns_early_when_labels_disabled() {
    let meter = opentelemetry::global::meter("docker-test-no-labels");
    let instruments = Instruments::new(&meter);
    let filter = PrefixFilter::new(vec!["system.docker.".to_string()], vec![]);
