...
fn unique_temp_path(name: &str) -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "ojo-docker-bin-test-{name}-{}-{nanos}",
        std::process::id()
    ))
}
...
fn binary_dump_snapshot_with_explicit_config_succeeds() {
    ...
    fs::write(
        &config,
        "service:\n  name: ojo-docker-integration\ncollection:\n  poll_interval_secs: 1\nexport:\n  otlp:\n    endpoint: http://127.0.0.1:4318/v1/metrics\n    protocol: http/protobuf\nstorage:\n  archive_enabled: false\n",
    )
    .expect("write config");

    let exe = std::env::var("CARGO_BIN_EXE_ojo-docker").expect("docker test binary path");
    let output = Command::new(exe)
        .arg("--config")
        .arg(config.to_string_lossy().to_string())
        .arg("--dump-snapshot")
        .output()
        .expect("run ojo-docker --dump-snapshot");
