use super::{
    collect_systemd_up, load_yaml_config_file, parse_bool_env, record_u64,
...
#[test]
fn collect_systemd_up_respects_env() {
    let _guard = env_lock().lock().expect("env lock");
    std::env::set_var("OJO_SYSTEMD_SIMULATE_UP", "1");
    assert_eq!(collect_systemd_up(), 1);
    std::env::set_var("OJO_SYSTEMD_SIMULATE_UP", "0");
    assert_eq!(collect_systemd_up(), 0);
