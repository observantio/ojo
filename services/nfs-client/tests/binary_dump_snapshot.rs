...
fn binary_dump_snapshot_with_explicit_config_succeeds() {
    ...
    fs::write(
        &config,
        "service:\n  name: ojo-nfs-integration\ncollection:\n  poll_interval_secs: 1\nnfs_client:\n  executable: /definitely/missing/nfsstat\nexport:\n  otlp:\n    endpoint: http://127.0.0.1:4318/v1/metrics\n    protocol: http/protobuf\n",
    )
    .expect("write config");

    let exe = std::env::var("CARGO_BIN_EXE_ojo-nfs-client").expect("nfs test binary path");
    let output = Command::new(exe)
        .arg("--config")
        .arg(config.to_string_lossy().to_string())
        .arg("--dump-snapshot")
        .output()
        .expect("run ojo-nfs-client --dump-snapshot");
...
fn binary_run_once_with_explicit_config_succeeds() {
    ...
    let output = Command::new(exe)
        ...
        .env("OJO_RUN_ONCE", "1")
        .output()
        .expect("run ojo-nfs-client once");
