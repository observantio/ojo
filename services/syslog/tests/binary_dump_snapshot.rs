...
fn unique_temp_path(name: &str) -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "ojo-syslog-bin-test-{name}-{}-{nanos}",
        std::process::id()
    ))
}
...
fn binary_dump_snapshot_with_explicit_config_succeeds() {
    ...
    fs::write(
        &config,
        "service:\n  name: ojo-syslog-integration\ncollection:\n  poll_interval_secs: 1\n  max_lines_per_source: 1\n  max_message_bytes: 256\nwatch:\n  files: []\nexport:\n  otlp:\n    endpoint: http://127.0.0.1:4318/v1/metrics\n    protocol: http/protobuf\n  logs:\n    endpoint: http://127.0.0.1:4318/v1/logs\n    timeout_secs: 1\nstorage:\n  archive_enabled: false\n",
    )
    .expect("write config");

    let exe = std::env::var("CARGO_BIN_EXE_ojo-syslog").expect("syslog test binary path");
    let output = Command::new(exe)
        .arg("--config")
        .arg(config.to_string_lossy().to_string())
        .arg("--dump-snapshot")
        .output()
        .expect("run ojo-syslog --dump-snapshot");
